...
namespace HITS.LIB.WeakEvents
{
    ...
    /// Define the StandardMessage class inheriting from ValueChangedMessage
    ...
    /// Key Features:
    /// Generic Type(T): The type parameter T represents the type of the value that has changed.In this case, it's IEventData.
    /// Constructor: The class has a constructor that accepts the changed value(eventData) and initializes the message with it.
    ...
    public class StandardMessage : ValueChangedMessage<IEventData>
    {
        // Constructor for StandardMessage, accepting IEventData as a parameter
        public StandardMessage(IEventData eventData) : base(eventData)
        {
